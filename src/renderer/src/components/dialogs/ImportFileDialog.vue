...
import { detectImportWellName, importData } from '@/api/data'
...
const canSubmit = computed(() => {
  if (!form.dataType || !form.filePath)
    return false
  if (needsWellName.value && !form.wellName.trim())
    return false
  return true
})
...
function resetForm() {
  ...
  form.filePath = dialogStore.importPresetFilePath || ''
  form.wellName = dialogStore.importPresetWellName || ''
}

async function maybeAutofillWellName() {
  if (!form.dataType || !form.filePath || form.wellName.trim())
    return

  try {
    const res = await detectImportWellName({
      file_path: form.filePath,
      data_type: form.dataType,
    })
    if (!form.wellName.trim() && res.well_name) {
      form.wellName = res.well_name
    }
  }
  catch {
    // Ignore auto-detect failures; manual input remains available.
  }
}
...
<template>
  ...
  >
    <el-form :model="form" label-width="80px">
      ...
      <el-form-item label="井名">
        <el-input
          ...
          :placeholder="needsWellName ? '请输入井名' : '可手动填写，选文件后自动识别'"
        />
      </el-form-item>
    ...
    <template #footer>
      <el-button @click="dialogStore.importFileVisible = false">
        取消
      </el-button>
      <el-button
        type="primary"
